...
def shoot_bullet_at_certain_angle(bullet, screen):
    for projectile in bullet.bullets:
        if not projectile.bulletFire:
            ...
            dx = mouse_x+8 - projectile.x
            dy = mouse_y-16 - projectile.y
            angle = math.atan2(dx, dy)
            projectile.xChange = (math.sin(angle) * projectile.speed)
            projectile.yChange = (math.cos(angle) * projectile.speed)
        ...
        if bullet.mother:
            bullet.owner.equipped_weapon.bullets.append(
                arsenal.Spell(bullet.img_name, bullet.owner, bullet.min_dmg, bullet.max_dmg,
...
def shoot_bomb(bomb, screen):
    if not bomb.fire:
        ...
        dy = mouse_y - bomb.y
        angle = math.atan2(dx, dy)
        bomb.xChange = (math.sin(angle) * bomb.speed)
        bomb.yChange = (math.cos(angle) * bomb.speed)
