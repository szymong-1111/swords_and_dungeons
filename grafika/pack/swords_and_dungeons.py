...
class Player:
    def __init__(self, img, x, y, maxHp, maxMana, equipment, equipped_weapon=None, speed=5, is_player=False):
        self.maxHp = maxHp
        self.hp = maxHp
    ...
    def move(self):
        """
        moves the player and updates it's hitbox
        """
    ...
    def is_selected(self):
        """
        displays the select frame if player is selected
        """
...
class Ray:
    ...
    def display(self):
        self.rect.update(self.x, self.y, 80, 80)
...
def new_dungeon():
    ...
    for _ in range(random.randint(3, 6)):
        ...
        chest_list.append(
            chests.Chest('chest_demo2.png', chest_spawnpoint[0], chest_spawnpoint[1], chests.basic_chest_loot))
...
while running:
    ...
    for _ in range(4):
        '''
        for wall in algorithm.walls:
            if Ray(player.x+80, player.y).rect.colliderect(wall):
                break
        '''
    for path in algorithm.paths:
    ...
    for weapon in player.equipment:
        ...
        elif type(weapon) is potions.Potion:
            weapon.x = player.x
            weapon.y = player.y
    ...
    for chest in chest_list:
        ...
        if chest.show_loot:
            ...
            elif counter == chest.saved_count+2 or counter == chest.saved_count+3:
                screen.blit(chest.loot.img, (chest.x + (64-chest.loot.img.get_width())/2, chest.y - 20))
            elif counter == chest.saved_count+4 or counter == chest.saved_count+5:
                screen.blit(chest.loot.img, (chest.x + (64-chest.loot.img.get_width())/2, chest.y - 40))
            elif counter == chest.saved_count+6 or counter == chest.saved_count+7:
                screen.blit(chest.loot.img, (chest.x + (64-chest.loot.img.get_width())/2, chest.y - 60))
            elif counter == chest.saved_count+8 or counter == chest.saved_count+9 or counter == chest.saved_count+10 or\
                    counter == chest.saved_count+11:
                screen.blit(chest.loot.img, (chest.x + (64-chest.loot.img.get_width())/2, chest.y - 80))
            elif counter == chest.saved_count+12 or counter == chest.saved_count+13:
                screen.blit(chest.loot.img, (chest.x + (64-chest.loot.img.get_width())/2, chest.y - 60))
            elif counter == chest.saved_count+14 or counter == chest.saved_count+15:
                screen.blit(chest.loot.img, (chest.x + (64-chest.loot.img.get_width())/2, chest.y - 40))
            elif counter == chest.saved_count+16 or counter == chest.saved_count+17:
                screen.blit(chest.loot.img, (chest.x + (64-chest.loot.img.get_width())/2, chest.y - 20))
            elif counter == chest.saved_count+18:
                screen.blit(chest.loot.img, (chest.x + (64-chest.loot.img.get_width())/2, chest.y))
    ...
    try:
        if type(player.equipped_weapon) is arsenal.Spell:
            ...
            screen.blit(player.equipped_weapon.img, (0, 536))
